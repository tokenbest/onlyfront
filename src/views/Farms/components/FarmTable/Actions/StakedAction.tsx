...
import { Button, useModal, IconButton, AddIcon, MinusIcon } from '@tokenbest/uikit'
...
const Staked: React.FunctionComponent<FarmWithStakedValue> = ({ pid, lpSymbol, lpAddresses, quoteToken, token }) => {
  ...
  const rawStakedFreeAmount = getBalanceNumber(stakedFreeAmount)
  const displayFreeBalance = rawStakedFreeAmount.toLocaleString()

  const [onPresentDeposit] = useModal(
    <DepositModal max={tokenBalance} onConfirm={onStake} tokenName={lpSymbol} addLiquidityUrl={addLiquidityUrl} />,
  )

  const [onPresentWithdraw] = useModal(<WithdrawModal max={stakedFreeAmount} onConfirm={onUnstake} tokenName={lpSymbol} stakedLocks={stakedLocks} />)
  ...
  if (!account) {
    return (
      <ActionContainer>
        <ActionTitles>
          <Subtle style={{ color: '#3D6BF3', fontSize: window.innerWidth > 750 ? '20px' : '16px' }}>{TranslateString(999, 'START FARMING')}</Subtle>
        </ActionTitles>
        <ActionContent>
